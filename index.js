    ...
    try {
      const image = await nodeHtmlToImage({
        html: `
            ...
            <div style="width:700px;height:700px;background:black;display:flex;align-self:center;justify-content:center"><div style="height:540px;background:black;align-self:center;width:615px;position:relative"><div style="width:129px;height:134px;position:absolute;right:1rem;top:0rem;background:black;background-repeat:no-repeat;background-position:center;background-size:cover;background-image:url(&quot;${
              x.dao_logo_url
            }&quot;)"><div style="height:0;width:0;border-bottom:30px solid black;border-right:30px solid transparent;bottom:0;position:absolute"></div><div style="height:0;width:0;border-bottom:36px solid transparent;border-right:36px solid black;top:0;position:absolute;right:0"></div></div><img alt="pocp_bg" src="https://firebasestorage.googleapis.com/v0/b/eveels-c43bb.appspot.com/o/POCP_background.svg?alt=media&amp;token=629aa0c3-e949-4117-bdf1-4b3c0b53a756" style="position:absolute;width:100%;height:100%;top:0;right:0"/><div style="font-size:18px;color:white;font-family:SFMono;left:0;position:absolute;top:0.3rem">${
          x.date_of_approve
        }</div><div style="width:70%;position:absolute;background:red"><div style="font-size:52px;font-family:TelegrafMedium;left:30px;position:absolute;top:76px;font-weight:400;line-height:52px;color:#F5A60B;text-align:start">${
          x.dao_name
        }</div><div style="font-size:52px;font-family:TelegrafMedium;left:30px;position:absolute;top:128px;font-weight:400;line-height:52px;color:black;text-align:start">${
          x.contri_title
        }</div></div><div style="display:flex;flex-direction:row;position:absolute;top:256px;left:30px"><div><div style="font-weight:500;font-size:24px;color:black;font-family:SFMono;text-align:start">To</div><div style="font-weight:500;font-size:24px;color:black;font-family:SFMono">From</div></div><div><div style="font-weight:500;font-size:24px;color:black;font-family:SFMono">${x.claimer?.slice(
          0,
          5
        )}...${x?.claimer?.slice(
          -3
        )}</div><div style="font-weight:500;font-size:24px;color:black;font-family:SFMono">${x.signer?.slice(
          0,
          5
        )}...${x?.signer?.slice(
          -3
        )}</div></div></div><div style="display:flex;flex-direction:row;position:absolute;bottom:18px;left:160px;justify-content:space-between;right:56px;align-items:center"><div style="font-weight:500;font-size:18px;color:#767676;font-family:SFMono">DREP-${
          x.id
        }</div><div style="font-weight:500;font-size:18px;color:black;font-family:SFMono">PROOF OF CONTRIBUTION</div></div></div></div>
            </html>`,
      });
